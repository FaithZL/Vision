...
void ReSTIRDirectIllumination::compile_shader0() noexcept {
    ...
    _shader0 = device().async_compile(ocarina::move(kernel), "ReSTIR direct initial candidates and temporal reuse");
}
...
void ReSTIRDirectIllumination::compile_shader1() noexcept {
    ...
    _shader1 = device().async_compile(ocarina::move(kernel),
                                      "ReSTIR direct spatial reuse and shading");
}
