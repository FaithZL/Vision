...
CameraImpl::CameraImpl(const SensorDesc &desc)
    : SensorImpl(desc) {
    init(desc);
    _update_resolution(film_->resolution());
}

void CameraImpl::init(const SensorDesc &desc) noexcept {
...
void CameraImpl::render_sub_UI(ocarina::Widgets *widgets) noexcept {
...
RayState CameraImpl::generate_ray(const SensorSample &ss) const noexcept {
...
RayVar CameraImpl::generate_ray_in_camera_space(const vision::SensorSample &ss) const noexcept {
...
void CameraImpl::set_resolution(ocarina::uint2 res) noexcept {
    SensorImpl::set_resolution(res);
    _update_resolution(res);
}

void CameraImpl::_update_resolution(uint2 res) noexcept {
...
void CameraImpl::_update_raster() noexcept {
...
void CameraImpl::update_mat(float4x4 m) noexcept {
...
void CameraImpl::set_mat(ocarina::float4x4 m) noexcept {
    c2w_ = m;
    position_ = make_float3(m[3]);
}

void CameraImpl::after_render() noexcept {
    store_prev_data();
}

void CameraImpl::store_prev_data() noexcept {
    prev_c2r_ = inverse(raster_to_camera_.hv());
    prev_w2c_ = inverse(c2w_.hv());
    prev_pos_ = position();
}

Float3 CameraImpl::prev_raster_coord(Float3 pos) const noexcept {
...
Float3 CameraImpl::raster_coord(ocarina::Float3 pos) const noexcept {
...
void CameraImpl::update_device_data() noexcept {
    c2w_ = camera_to_world();
    SensorImpl::update_data();
    upload_immediately();
}

void CameraImpl::prepare() noexcept {
    SensorImpl::prepare();
    prepare_data();
    upload_immediately();
    store_prev_data();
...
Float CameraImpl::linear_depth(const Float3 &world_pos) const noexcept {
    Float4x4 w2c = inverse(device_c2w());
