...
def generate(file, dim):
    ...
    if dim == 1:
        for i, x in enumerate(input):
            str = f"case 0x{i}: return node_->evaluate(ctx, swl).{x}_();"
            print(str, file=file)
    elif dim == 2:
        for i, x in enumerate(input):
            for j, y in enumerate(input):
                str = f"case 0x{i}{j}: return node_->evaluate(ctx, swl).{x}{y}_();"
                print(str, file=file)
    elif dim == 3:
        for i, x in enumerate(input):
            for j, y in enumerate(input):
                for k, z in enumerate(input):
                    str = f"case 0x{i}{j}{k}: return node_->evaluate(ctx, swl).{x}{y}{z}_();"
                    print(str, file=file)
    else:
        for i, x in enumerate(input):
            for j, y in enumerate(input):
                for k, z in enumerate(input):
                    for l, w in enumerate(input):
                        str = f"case 0x{i}{j}{k}{l}: return node_->evaluate(ctx, swl).{x}{y}{z}{w}_();"
                        print(str, file=file)
