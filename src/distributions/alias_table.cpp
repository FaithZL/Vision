...
public:
    ...
    [[nodiscard]] tuple<Int, Float> offset_u_remapped(Float u) const noexcept {
        u = u * float(size());
        Int offset = min(cast<int>(u), int(size() - 1));
    ...
    [[nodiscard]] tuple<Float, Float, Int> sample_continuous(Float u) const noexcept override {
        auto [offset, u_remapped] = offset_u_remapped(u);
        Float ret = (offset + u_remapped) / float(size());
        return {ret, PDF(offset), offset};
    }
    [[nodiscard]] tuple<Int, Float, Float> sample_discrete(Float u) const noexcept override {
        auto [offset, u_remapped] = offset_u_remapped(u);
